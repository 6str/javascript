...
function getHosts2(data){
    // reduce the podcasts data down to a list of hosts
    return data.reduce((acc, {hosts}) => acc.concat(hosts), [])
 }

function assignAwards(data){
   // assign random award to each host
   let hosts = getHosts2(data) 
   
   return hosts.map(host => {
        let award = awards[Math.floor(Math.random() * awards.length)]
        return award + ' ' + host
    })
}
